      ...
      <div className="mx-auto max-w-4xl px-4 mt-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2">
              ...
              <input value={subject} onChange={(e)=>setSubject(e.target.value)} placeholder="contoh: Order belum masuk" className="mt-1 w-full border border-slate-300 rounded px-3 py-2" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700">Order ID (opsional)</label>
              <input value={orderId} onChange={(e)=>setOrderId(e.target.value)} placeholder="contoh: INV-12345" className="mt-1 w-full border border-slate-300 rounded px-3 py-2" />
            </div>
            <div className="sm:col-span-3">
              <label className="block text-sm font-medium text-slate-700">Pesan</label>
              <textarea value={message} onChange={(e)=>setMessage(e.target.value)} rows={4} className="mt-1 w-full border border-slate-300 rounded px-3 py-2" placeholder="Jelaskan kendala kamu" />
            </div>
        ...
        items.map((t) => (
          ...
          <td className="px-2 py-2 font-mono text-xs"><Link href={`/tickets/${t.ticketId}`} className="text-indigo-600">{t.ticketId}</Link></td>
                    <td className="px-2 py-2"><Link href={`/tickets/${t.ticketId}`} className="text-slate-800 hover:underline">{t.subject}</Link></td>
                    <td className="px-2 py-2"><span className="px-2 py-0.5 rounded bg-slate-100 text-slate-700 text-xs uppercase">{t.status}</span></td>
                    <td className="px-2 py-2">{formatDate(t.updatedAt)}</td>
                  </tr>
                ))
              )}
            </tbody>
