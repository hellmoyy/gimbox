        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2">
              ...
              <input value={subject} onChange={(e)=>setSubject(e.target.value)} placeholder="contoh: Order belum masuk" className="mt-1 w-full border border-slate-300 rounded px-3 py-2 text-slate-800 placeholder-slate-400" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700">Order ID (opsional)</label>
              <input value={orderId} onChange={(e)=>setOrderId(e.target.value)} placeholder="contoh: INV-12345" className="mt-1 w-full border border-slate-300 rounded px-3 py-2 text-slate-800 placeholder-slate-400" />
            </div>
            <div className="sm:col-span-3">
              <label className="block text-sm font-medium text-slate-700">Pesan</label>
              <textarea value={message} onChange={(e)=>setMessage(e.target.value)} rows={4} className="mt-1 w-full border border-slate-300 rounded px-3 py-2 text-slate-800 placeholder-slate-400" placeholder="Jelaskan kendala kamu" />
            </div>
