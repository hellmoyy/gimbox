...
export default async function GameEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!creating) {
    try {
      item = await db.collection("games").findOne({ _id: new ObjectId(id) });
    } catch {}
  }
  const action = creating ? "/api/admin/games" : `/api/admin/games/${id}`;

  return (
    <main>
      <div className="max-w-2xl">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-xl font-semibold text-slate-900">{creating ? "Tambah Game" : `Edit: ${item?.name || "Game"}`}</h1>
          <Link href="/admin/games" className="text-sm text-slate-700">← Kembali</Link>
        </div>
  <form action={action} method="post" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <div className="mt-2">
                <ProductImageUploader defaultFolder="games" defaultName={item?.code || item?.name || "game"} onUploadedFieldName="icon" />
              </div>
            </div>
            <div>
