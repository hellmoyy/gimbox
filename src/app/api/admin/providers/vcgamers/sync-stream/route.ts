...
import { getBrands, getBrandProducts } from '@/lib/providers/vcgamers';
...
export async function GET(req: NextRequest) {
  ...
  send({ type: 'brand:start', key: b.key, canonical: canonicalCode, name: b.name, index: brandIndex, total: brands.length });
        ...
        try {
          ...
          send({ type: 'brand:error', key: b.key, canonical: canonicalCode, message: e?.message || 'fetch failed' });
          continue;
        }
        ...
        for (const p of list) {
          ...
          const code = `${canonicalCode}-${p.providerProductCode}`.toLowerCase();
            ...
            const existing = await db.collection('products').findOne({ code }, { projection: { customPrice:1, hash:1, price:1 } });
            ...
            await db.collection('products').updateOne({ code }, { $set: update, $setOnInsert: { createdAt: new Date() } }, { upsert: true });
            upserted++; processed++;
