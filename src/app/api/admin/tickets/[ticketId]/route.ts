...
export async function GET(req: NextRequest, { params }: { params: { ticketId: string } }) {
  try {
    ...
    const row = await db.collection("tickets").findOne({ ticketId: params.ticketId }, { projection: { _id: 0 } });
...
export async function POST(req: NextRequest, { params }: { params: { ticketId: string } }) {
  try {
    ...
    const res = await db.collection("tickets").updateOne({ ticketId: params.ticketId }, update);
