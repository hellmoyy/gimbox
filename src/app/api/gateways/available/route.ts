...
export async function GET() {
  ...
  const keys = ["gateway:midtrans", "gateway:xendit", "gateway:moota", "gateway:duitku"];
  const docs = await db.collection("settings").find({ key: { $in: keys } }).toArray();
    ...
    const data = [
      ...
      { name: "duitku", enabled: Boolean(map["gateway:duitku"]?.enabled), methods: Array.isArray(map["gateway:duitku"]?.methods) ? map["gateway:duitku"].methods : [] },
    ];
